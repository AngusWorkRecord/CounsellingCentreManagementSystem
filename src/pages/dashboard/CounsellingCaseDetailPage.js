...
export default function CounsellingCaseDetailPage() {
  ...
  return (
    <>
      {/* 中文原文：个案详情 | 辅导中心 */}
      <Helmet><title>{tr("Case Details | Counselling Centre")}</title></Helmet>
      <Container maxWidth={false}>
        ...
        >
          <Box>
            ...
            {/* 中文原文：个案详情 */}
            <Typography variant="h3" gutterBottom>{tr("Case Details")}</Typography>
          ...
          <Stack alignItems={{ xs: 'stretch', sm: 'flex-end' }} spacing={1.5}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
              <Button variant="outlined" size="large" startIcon={<Iconify icon="eva:arrow-back-fill" />} onClick={goBack}>
                {/* 中文原文：返回个案列表 */}{tr("Back to Case List")}</Button>
              {session?.report_url && (
                <Button
                  component="a"
                ...
                >
                  {/* 中文原文：查看详细报告 */}{tr("View Detailed Report")}</Button>
              )}
            </Stack>
          </Stack>
        </Stack>
        ...
        {!loading && error && (
          <Alert
            severity="error"
            action={<Button color="inherit" size="small" onClick={() => setReloadKey((value) => value + 1)}>{tr("Reload")}</Button>}
          >
            {uiMessage(error)}
          </Alert>
        )}
        ...
        {!loading && !error && (!validId || !session) && (
          <Alert
            severity="warning"
            action={<Button color="inherit" size="small" onClick={goBack}>{tr("Back to List")}</Button>}
          >
            {/* 中文原文：找不到指定 ID 的辅导个案。 */}{tr("Counselling case with ID “")}{id || '-'}{tr("” was not found.")}</Alert>
        )}
