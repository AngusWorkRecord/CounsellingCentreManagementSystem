    ...
    <Box sx={{ border: 1, borderColor: 'divider', borderRadius: 2, overflow: 'hidden' }}>
      <Box sx={{ p: { xs: 2, md: 3 }, pb: dense ? { xs: 1, md: 1.5 } : undefined }}>
        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: dense ? 1 : 2 }}>
          ...
          {/* 中文原文：详细报告待跟进提醒 */}<Typography variant="h6">{tr("Detailed Report Follow-up Reminders")}</Typography>
        </Stack>
        {!cases.length && (
          <Stack alignItems="center" spacing={1} sx={{ py: 3, color: 'text.secondary' }}>
            ...
            {/* 中文原文：当前没有达到提醒时间的详细报告 */}<Typography variant="body2">{tr("No detailed reports have reached the reminder threshold")}</Typography>
          </Stack>
        )}
            ...
            <Box key={item.id}>
              ...
              >
                ...
                <Box sx={{ minWidth: 225 }}>
                  ...
                  {/* 中文原文：案主简称 */}<Typography variant="body2">{tr("Client Initials:")} {item.initials}</Typography>
                </Box>
                {/* 中文原文：辅导员、辅导结束、逾期时长、提醒等级、详细报告、待完成、提醒状态、已提醒、未提醒 */}
                <Info label={tr("Counsellor")} value={item.counsellor} />
                <Info label={tr("Session Ended")} value={item.sessionEndedAt ? item.sessionEndedAt.toLocaleString(currentLocale()) : '-'} />
                <Info label={tr("Overdue By")} value={item.elapsedSinceEnd} />
                <Info label={tr("Reminder Level")} value={domainLabel(item.reminderStage)} color={`${color}.main`} />
                <Info label={tr("Detailed Report")} value={tr('Pending')} color="error.main" />
                <Info label={tr("Reminder Status")} value={sent ? tr('Sent') : tr('Not Sent')} color={sent ? 'success.main' : 'warning.main'} />
                <Box sx={{ flexGrow: 1 }} />
                <Button
                  variant="outlined"
                  disabled={sent}
                ...
                >
                  {/* 中文原文：已提醒、发送提醒 */}{sent ? tr("Sent") : tr("Send Reminder")}
                </Button>
              </Stack>
            </Box>
          );
