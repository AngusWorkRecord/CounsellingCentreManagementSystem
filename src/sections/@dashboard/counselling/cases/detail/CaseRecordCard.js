...
export default function CaseRecordCard({ session }) {
  ...
  const rows = [
    ...
    ['Report Status', session.report_completed ? tr('Completed') : tr('Incomplete')],
    ['Notification Status', session.notification_sent ? tr('Sent') : tr('Not Sent')],
    ...
    <Card sx={{ p: 3 }}>
      ...
      <Box sx={{ border: 1, borderColor: 'divider', borderRadius: 1.5, overflow: 'hidden' }}>
        {rows.map(([label, value]) => (
          <Box key={label}>
            <Grid container>
              <Grid item xs={5} sm={4} sx={{ p: 1.5, bgcolor: 'background.neutral' }}><Typography variant="subtitle2">{tr(label)}</Typography></Grid>
        ...
        <Grid container>
          <Grid item xs={5} sm={4} sx={{ p: 1.5, bgcolor: 'background.neutral' }}><Typography variant="subtitle2">{tr("Detailed Report Link")}</Typography></Grid>
